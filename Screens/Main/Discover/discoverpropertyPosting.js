...
import { ModalView, Heading, ButtonContainer, ImageContainer, NextContainer,InfoText, ContinueButton,
    ...
    FollowUpContainer, FollowUpText, DateCategoryName, BedroomContaienr,BedroomItemContainer, RowContainerCol} from './discoverPropertyPostingStyle';
...
export default function PropertyPostingScreen({navigation}){
    ...
    const flatListdata =[{name: "Room", image: require('../../../assets/room.jpeg'), description: "Shared public space" },
    ...
    function SelectLocationSequence(){
        ...
        Animated.sequence([
            ...
            Animated.spring(SearchHeightTranslation,{
                toValue: HEIGHT*0.4,
                bounciness:0,
                useNativeDriver:false,
            })
    ...
    return(
       <View style={{width:WIDTH, height:HEIGHT, margin: 0, padding:0}} >
            <ModalView>
                ...
                style={{opacity: OpacityTranslation.interpolate({
                    inputRange:[0, 1],
                    outputRange:[0,1]
                })}}
                scrollEnabled={false}  horizontal snapToAlignment='end' decelerationRate='fast' snapToInterval={WIDTH} ref={scrollView} onTouchEnd={()=>setexpended(true)} >
                    ...
                    <PostingSection >
                        <Animated.View style={{height: TopContainerTranslation 
                        ...
                        <Animated.View style={{width:WIDTH*0.9, height:SearchHeightTranslation,  borderRadius:10}}>
                           
                            {propertyLocation.length != 0 && autocompleteLocation.map((value)=>(
                                <Pressable key={value.description} onPress={()=> {setpropertyLocation(value.description), Keyboard.dismiss(), setautocompleteLocation([]), moveScrollView(scrollviewIndex+1)}} >
                                    <View style={{width:WIDTH*0.9, height:HEIGHT*0.08,  paddingLeft: WIDTH*0.025,
                    ...
                    <PostingSection>
                        ...
                        {test.map((value, index)=>(
                            ...
                            <TouchableOpacity key={value + index} onPress={()=>selectGallery(index)}>
                            <PhotoContainer>
                            ...
                            {amenitiesList.map((value,index)=>(
                                <View key={value.name + 'view'} style={{minWidth: WIDTH*0.35, width: value.name.length*0.03*WIDTH, height:HEIGHT*0.055, justifyContent:'center', 
                                 paddingRight: WIDTH*0.03}}>
                                    <Pressable key={value.name + 'pressable'} onPress={()=>updateAmenities(value.name)} style={{borderWidth:3, borderColor: propertyAmenities.indexOf(value.name) == -1 ? value.color : '#0085FF', height: HEIGHT*0.045, 
                    ...
                    <ScrollView style={{paddingLeft:WIDTH*0.05}}>
                        ...
                        {test.map((value, index)=>(
                            propertyphotoGallery[index]=="" || propertyphotoGallery[index]== null ? 
                            <TouchableOpacity key={index+ "null"} onPress={()=>selectGallery(index)}>
                            <PhotoContainer>
                                <Ionicons name="add" size={20}/>
                            </PhotoContainer>
                            </TouchableOpacity>
                            :
                            <TouchableOpacity  key={index + "notNull"} onPress={()=>selectGallery(index)}>
                                <PhotoContainer >
                                </PhotoContainer>
                            </TouchableOpacity>
                        ))}
                ...
                <NextContainer>
                    <ContinueButton style={{display: scrollviewIndex == 10 ? 'none' : 'flex'}} onPress={()=>moveScrollView(scrollviewIndex+1)}>
                        <ContinueText>
                            Next
                        </ContinueText>
                    </ContinueButton>
